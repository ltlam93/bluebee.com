...
$tbl_post_add = NULL; // Initialize page object first

class ctbl_post_add extends ctbl_post {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["post_id"] != "") {
				...
				$this->setKey("post_id", $this->post_id->CurrentValue); // Set up key
			} else {
				$this->setKey("post_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "tbl_postview.php")
	...
	function LoadDefaultValues() {
		$this->post_author->CurrentValue = NULL;
		$this->post_author->OldValue = $this->post_author->CurrentValue;
		$this->post_date->CurrentValue = NULL;
		$this->post_date->OldValue = $this->post_date->CurrentValue;
		$this->post_content->CurrentValue = NULL;
		$this->post_content->OldValue = $this->post_content->CurrentValue;
		$this->post_title->CurrentValue = NULL;
		$this->post_title->OldValue = $this->post_title->CurrentValue;
		$this->post_active->CurrentValue = NULL;
		$this->post_active->OldValue = $this->post_active->CurrentValue;
		$this->post_rate->CurrentValue = NULL;
		$this->post_rate->OldValue = $this->post_rate->CurrentValue;
		$this->post_type->CurrentValue = NULL;
		$this->post_type->OldValue = $this->post_type->CurrentValue;
		$this->post_class->CurrentValue = NULL;
		$this->post_class->OldValue = $this->post_class->CurrentValue;
		$this->post_group->CurrentValue = NULL;
		$this->post_group->OldValue = $this->post_group->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// post_author
		$this->post_author->SetDbValueDef($rsnew, $this->post_author->CurrentValue, NULL, FALSE);

		// post_date
		$this->post_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->post_date->CurrentValue, 5), NULL, FALSE);

		// post_content
		$this->post_content->SetDbValueDef($rsnew, $this->post_content->CurrentValue, NULL, FALSE);

		// post_title
		$this->post_title->SetDbValueDef($rsnew, $this->post_title->CurrentValue, NULL, FALSE);

		// post_active
		$this->post_active->SetDbValueDef($rsnew, $this->post_active->CurrentValue, NULL, FALSE);

		// post_rate
		$this->post_rate->SetDbValueDef($rsnew, $this->post_rate->CurrentValue, NULL, FALSE);

		// post_type
		$this->post_type->SetDbValueDef($rsnew, $this->post_type->CurrentValue, NULL, FALSE);

		// post_class
		$this->post_class->SetDbValueDef($rsnew, $this->post_class->CurrentValue, NULL, FALSE);

		// post_group
		$this->post_group->SetDbValueDef($rsnew, $this->post_group->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$this->post_id->setDbValue($conn->Insert_ID());
			$rsnew['post_id'] = $this->post_id->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($tbl_post_add)) $tbl_post_add = new ctbl_post_add();

// Page init
$tbl_post_add->Page_Init();

// Page main
$tbl_post_add->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var tbl_post_add = new ew_Page("tbl_post_add");
tbl_post_add.PageID = "add"; // Page ID
var EW_PAGE_ID = tbl_post_add.PageID; // For backward compatibility

// Form object
var ftbl_postadd = new ew_Form("ftbl_postadd");

// Validate form
ftbl_postadd.Validate = function(fobj) {
...
ftbl_postadd.Form_CustomValidate =  
...
ftbl_postadd.ValidateRequired = true;
<?php } else { ?>
ftbl_postadd.ValidateRequired = false; 
<?php } ?>
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $tbl_post->TableCaption() ?></span></p>
<p class="phpmaker"><a href="<?php echo $tbl_post->getReturnUrl() ?>" id="a_GoBack" class="ewLink"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $tbl_post_add->ShowPageHeader(); ?>
<?php
$tbl_post_add->ShowMessage();
?>
<form name="ftbl_postadd" id="ftbl_postadd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="tbl_post">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_tbl_postadd" class="ewTable">
