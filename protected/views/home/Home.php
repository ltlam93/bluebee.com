
        <div class = "l-submain color_primary type_fullwidth border_none">
            <div class = "l-submain-h g-html">
                <div class = "g-cols">
                    <div class = "full-width">

                        <div class = "fullwidthbanner-container">
                            <div class = "fullwidthbanner">
                                <ul>
                                    <li data-transition = "fade">
                                        <div class = "caption lft" data-x = "center" data-y = "100" data-speed = "400" data-start = "600" data-easing = "easeInOutSine"><img style="width : 1200px;height: 350px" src="<?php echo Yii::app()->theme->baseUrl; ?>/assets/img/demo/IMG_0229_1.JPG" alt = "Image" /></div>
                                        <div class = "caption astra_white_heading_1 sft" data-x = "center" data-y = "40" data-speed = "600" data-start = "1000" data-easing = "easeInOutExpo">BlueBee - Một trải nghiệm học tập mới của UET</div>
                                    </li>
                                    <li data-transition = "curtain-1">
                                        <img src = "img/slides/transparent.png" alt = "" />

                                        <div class = "caption sfb" data-x = "720" data-y = "80" data-speed = "800" data-start = "600" data-easing = "easeInOutSine"><img src="<?php echo Yii::app()->theme->baseUrl; ?>/assets/img/demo/tiensy.jpg" alt = "Image" /></div>
                                        <div class = "caption sfl" data-x = "519" data-y = "169" data-speed = "800" data-start = "800" data-easing = "easeInSine"><img src="<?php echo Yii::app()->theme->baseUrl; ?>/assets/img/demo/lab6photo_1.jpg" alt = "Image" /></div>
                                        <div class = "caption astra_white_heading_1 sfr" data-x = "0" data-y = "130" data-speed = "300" data-start = "1000" data-easing = "easeInOutExpo">Kho lưu trữ tài liệu UET</div>
                                        <div class = "caption astra_white_medium_text sfl" data-x = "0" data-y = "210" data-speed = "300" data-start = "1200" data-easing = "easeInOutSine">Dữ liệu phong phú và đầy đủ<br />Nơi tổng hợp tất cả các tài liệu của các anh chị đi trước.</div>
                                        

                                    </li>
                                </ul>
                            </div>
                        </div>

                    </div>
                </div>

            </div>
        </div>

<div class = "l-submain color_alternate" style="background-color: #ffffff">
            <div class = "l-submain-h g-html">

                <div class = "w-actionbox controls_aside color_alternate">
                    <div class = "w-actionbox-h">
                        <div class = "w-actionbox-text">
                            <h3>BlueBee - Một trải nghiệm học tập mới của UET</h3>
                        </div>
                        <div class = "w-actionbox-controls">
                            <a class = "w-actionbox-button g-btn size_big type_primary" href = "https://www.facebook.com/hotrohoctapUET?fref=ts" target = "_blank"><span>Fanpage</span></a>

                        </div>
                    </div>
                </div>

            </div>
        </div>

        <div class = "l-submain">
            <div class = "l-submain-h g-html">

                <div class = "g-cols">
                    <div class = "one-third">
                        <div class = "w-iconbox icon_left">
                            <div class = "w-iconbox-h">
                                <div class = "w-iconbox-icon">
                                    <i class = "icon-rocket"></i>
                                </div>
                                <div class = "w-iconbox-text">
                                    <h3 class = "w-iconbox-text-title">Giao diện thân thiện</h3>
                                    <div class = "w-iconbox-text-description">
                                        <p>Chúng tôi đã sẽ và đang nỗ lực để biến website thân thiện, dễ sử dụng nhất với người dùng.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class = "one-third">
                        <div class = "w-iconbox icon_left">
                            <div class = "w-iconbox-h">
                                <div class = "w-iconbox-icon">
                                    <i class = "icon-magic"></i>
                                </div>
                                <div class = "w-iconbox-text">
                                    <h3 class = "w-iconbox-text-title">Tải tài liệu miễn phí</h3>
                                    <div class = "w-iconbox-text-description">
                                        <p>Đề thi, tài liệu, lab - tất cả chỉ có trên Bluebee.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class = "one-third">
                        <div class = "w-iconbox icon_left">
                            <div class = "w-iconbox-h">
                                <div class = "w-iconbox-icon">
                                    <i class = "icon-heart-empty"></i>
                                </div>
                                <div class = "w-iconbox-text">
                                    <h3 class = "w-iconbox-text-title">Trợ giúp học tập</h3>
                                    <div class = "w-iconbox-text-description">
                                        <p>Các bạn có thể hỏi tất cả những gì bạn muốn, chúng tôi sẽ trả lời.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

            </div>
        </div>

        <div class = "l-submain">
            <div class = "l-submain-h g-html i-cf">

                <div class = "g-cols">
                    <div class = "one-half animate_afl">
                        <div class = "wpb_single_image align_center">
                            <img src = "<?php echo Yii::app()->theme->baseUrl; ?>/assets/img/demo/uet.png" alt = "" />
                        </div>
                    </div>
                    <div class = "one-half">
                        <h2 style = "text-align: center;">Phát triển bởi sinh viên UET</h2>
                        <p style = "text-align: center;">Chúng tôi là những sinh viên tại trường đại học, là học sinh chuẩn bị bước vào năm nhất thời sinh viên.
                        Chúng tôi đã từng suy nghĩ và có nhu cầu về vấn đề môi trường học tập, tiếng nói của bản thân, những tài liệu cần thiết…</p>
                        <div class = "g-hr type_short size_small">
                            <span class = "g-hr-h">
                                <i class = "icon-star"></i>
                            </span>
                        </div>
                        <p style = "text-align: center;"><a class = "g-btn type_secondary" href = "<?php echo Yii::app()->createAbsoluteUrl('aboutUs')?>"><i class = "icon-chevron-right"></i>About Us</a></p>
                    </div>
                </div>

            </div>
        </div>

        <div class = "l-submain">
            <div class = "l-submain-h g-html i-cf">

                <div class = "g-cols">
                    <div class = "one-half">
                        <h2>Kho dữ liệu đầy đủ các ngành</h2>
                        <p>Với sự nhiệt tình của các bạn cộng tác viên, chúng tôi có tài liệu của tất cả các ngành học trong UET.Các bạn cũng có thể offer chúng tôi các tài liệu bạn cần, chúng tôi sẽ upload cho bạn.</p>
                        <div class = "g-hr type_invisible size_small">
                            <span class = "g-hr-h">
                                <i class = "icon-star"></i>
                            </span>
                        </div>
                        <a class = "g-btn type_outline" href = "<?php echo Yii::app()->createAbsoluteUrl('document')?>"><span><i class = "icon-laptop"></i>Đề thi - Tài liệu</span></a>
                    </div>
                    <div class = "one-half animate_afr">
                        <div class = "wpb_single_image align_center">
                            <img src = "<?php echo Yii::app()->theme->baseUrl; ?>/assets/img/demo/Books.jpg" alt = "" />
                        </div>
                    </div>
                </div>

            </div>
        </div>

        <div class = "l-submain">
            <div class = "l-submain-h g-html i-cf">

                <div class = "g-cols">
                    <div class = "one-half animate_afc">
                        <div class = "wpb_single_image align_center">
                            <img src = "<?php echo Yii::app()->theme->baseUrl; ?>/assets/img/demo/advice.jpg" alt = "" />
                        </div>
                    </div>
                    <div class = "one-half">
                        <h2>Lời khuyên của các anh chị đi trước</h2>
                        <p>Bluebee ko phải chỉ là 1 website, chúng tôi là 1 cộng đồng các khóa sv của UET. Chúng tôi có thể giúp đỡ, tư vấn các bạn 1 cách tận tình nhất về các thắc mắc học tập trong UET </p>
                        <div class = "g-hr type_invisible size_small">
                            <span class = "g-hr-h">
                                <i class = "icon-star"></i>
                            </span>
                        </div>
                        <a class = "g-btn type_primary" href = "<?php echo Yii::app()->createAbsoluteUrl('share/teacherListPage')?>"><span><i class = "icon-arrow-right"></i>Giáo viên</span></a>
                    </div>
                </div>

            </div>
        </div>

        <div class = "l-submain color_alternate">
            <div class = "l-submain-h g-html i-cf">

                <h2 style = "text-align: center;">Đánh giá về chúng tôi</h2>
                <div class = "g-hr type_short">
                    <span class = "g-hr-h">
                        <i class = "icon-heart"></i>
                    </span>
                </div>
                <div class = "g-cols">
                    <div class = "one-half animate_afl">
                        <div class = "w-testimonial">
                            <div class = "w-testimonial-h">
                                <blockquote>
                                    <q class = "w-testimonial-text">Một website rất hữu ích cho sinh viên mới vào trường !</q>
                                    <div class = "w-testimonial-person">
                                        <i class = "icon-user"></i>
                                        <span class = "w-testimonial-person-name">Trần Minh Tuấn</span>,
                                         <span class = "w-testimonial-person-meta">K59CLC</span>
                                    </div>
                                </blockquote>
                            </div>
                        </div>
                    </div>
                    <div class = "one-half animate_afr">
                        <div class = "w-testimonial">
                            <div class = "w-testimonial-h">
                                <blockquote>
                                    <q class = "w-testimonial-text">Sinh viên UET đang thiếu một nơi chia sẻ thông tin giáo viên, đề thi tài liệu chuyên nghiệp. Bluebee chính là giải pháp cho vấn đề này !</q>
                                    <div class = "w-testimonial-person">
                                        <i class = "icon-user"></i>
                                        <span class = "w-testimonial-person-name">Hoàng Hà</span>,
                                        <span class = "w-testimonial-person-meta">K57CA</span>
                                    </div>
                                </blockquote>
                            </div>
                        </div>
                    </div>
                </div>

            </div>
        </div>

        <div class = "l-submain color_primary">
            <div class = "l-submain-h g-html">

                <div class = "w-actionbox controls_aside color_primary">
                    <div class = "w-actionbox-h">
                        <div class = "w-actionbox-text">
                            <h3>Hãy trở thành BlueBee-er và share tài liệu cho cộng đồng</h3>
                        </div>
                    </div>
                </div>

            </div>
        </div>


