...
$tbl_program_view = NULL; // Initialize page object first

class ctbl_program_view extends ctbl_program {
	...
	// Page object name
	var $PageObjName = 'tbl_program_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["program_id"] <> "") {
			$this->RecKey["program_id"] = $_GET["program_id"];
			$KeyUrl .= "&program_id=" . urlencode($this->RecKey["program_id"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["program_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["program_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["program_id"] <> "") {
				...
				$this->RecKey["program_id"] = $this->program_id->QueryStringValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("tbl_programlist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->program_id->CurrentValue) == strval($this->Recordset->fields('program_id'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "tbl_programlist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "tbl_programlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_tbl_program\" href=\"javascript:void(0);\" onclick=\"ew_EmailDialogShow({lnk:'emf_tbl_program',hdr:ewLanguage.Phrase('ExportToEmail'),key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($tbl_program_view)) $tbl_program_view = new ctbl_program_view();

// Page init
$tbl_program_view->Page_Init();

// Page main
$tbl_program_view->Page_Main();
?>
...
// Page object
var tbl_program_view = new ew_Page("tbl_program_view");
tbl_program_view.PageID = "view"; // Page ID
var EW_PAGE_ID = tbl_program_view.PageID; // For backward compatibility

// Form object
var ftbl_programview = new ew_Form("ftbl_programview");

// Form_CustomValidate event
ftbl_programview.Form_CustomValidate =  
...
ftbl_programview.ValidateRequired = true;
<?php } else { ?>
ftbl_programview.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("View") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $tbl_program->TableCaption() ?>&nbsp;&nbsp;</span><?php $tbl_program_view->ExportOptions->Render("body"); ?>
</p>
<?php if ($tbl_program->Export == "") { ?>
<p class="phpmaker">
<a href="<?php echo $tbl_program_view->ListUrl ?>" id="a_BackToList" class="ewLink"><?php echo $Language->Phrase("BackToList") ?></a>&nbsp;
<?php if ($Security->IsLoggedIn()) { ?>
<?php if ($tbl_program_view->AddUrl <> "") { ?>
<a href="<?php echo $tbl_program_view->AddUrl ?>" id="a_AddLink" class="ewLink"><?php echo $Language->Phrase("ViewPageAddLink") ?></a>&nbsp;
<?php } ?>
<?php } ?>
<?php if ($Security->IsLoggedIn()) { ?>
<?php if ($tbl_program_view->EditUrl <> "") { ?>
<a href="<?php echo $tbl_program_view->EditUrl ?>" id="a_EditLink" class="ewLink"><?php echo $Language->Phrase("ViewPageEditLink") ?></a>&nbsp;
<?php } ?>
<?php } ?>
<?php if ($Security->IsLoggedIn()) { ?>
<?php if ($tbl_program_view->CopyUrl <> "") { ?>
<a href="<?php echo $tbl_program_view->CopyUrl ?>" id="a_CopyLink" class="ewLink"><?php echo $Language->Phrase("ViewPageCopyLink") ?></a>&nbsp;
<?php } ?>
<?php } ?>
<?php if ($Security->IsLoggedIn()) { ?>
<?php if ($tbl_program_view->DeleteUrl <> "") { ?>
<a onclick="return ew_Confirm(ewLanguage.Phrase('DeleteConfirmMsg'));" href="<?php echo $tbl_program_view->DeleteUrl ?>" id="a_DeleteLink" class="ewLink"><?php echo $Language->Phrase("ViewPageDeleteLink") ?></a>&nbsp;
<?php } ?>
<?php } ?>
</p>
<?php } ?>
<?php $tbl_program_view->ShowPageHeader(); ?>
<?php
$tbl_program_view->ShowMessage();
?>
<?php if ($tbl_program->Export == "") { ?>
<form name="ewpagerform" id="ewpagerform" class="ewForm" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager"><tr><td>
<?php if (!isset($tbl_program_view->Pager)) $tbl_program_view->Pager = new cPrevNextPager($tbl_program_view->StartRec, $tbl_program_view->DisplayRecs, $tbl_program_view->TotalRecs) ?>
<?php if ($tbl_program_view->Pager->RecordCount > 0) { ?>
	<table cellspacing="0" class="ewStdTable"><tbody><tr><td><span class="phpmaker"><?php echo $Language->Phrase("Page") ?>&nbsp;</span></td>
<!--first page button-->
	<?php if ($tbl_program_view->Pager->FirstButton->Enabled) { ?>
	<td><a href="<?php echo $tbl_program_view->PageUrl() ?>start=<?php echo $tbl_program_view->Pager->FirstButton->Start ?>"><img src="phpimages/first.gif" alt="<?php echo $Language->Phrase("PagerFirst") ?>" width="16" height="16" style="border: 0;"></a></td>
	<?php } else { ?>
	<td><img src="phpimages/firstdisab.gif" alt="<?php echo $Language->Phrase("PagerFirst") ?>" width="16" height="16" style="border: 0;"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($tbl_program_view->Pager->PrevButton->Enabled) { ?>
	<td><a href="<?php echo $tbl_program_view->PageUrl() ?>start=<?php echo $tbl_program_view->Pager->PrevButton->Start ?>"><img src="phpimages/prev.gif" alt="<?php echo $Language->Phrase("PagerPrevious") ?>" width="16" height="16" style="border: 0;"></a></td>
	<?php } else { ?>
	<td><img src="phpimages/prevdisab.gif" alt="<?php echo $Language->Phrase("PagerPrevious") ?>" width="16" height="16" style="border: 0;"></td>
	<?php } ?>
<!--current page number-->
	<td><input type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" id="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $tbl_program_view->Pager->CurrentPage ?>" size="4"></td>
<!--next page button-->
	<?php if ($tbl_program_view->Pager->NextButton->Enabled) { ?>
	<td><a href="<?php echo $tbl_program_view->PageUrl() ?>start=<?php echo $tbl_program_view->Pager->NextButton->Start ?>"><img src="phpimages/next.gif" alt="<?php echo $Language->Phrase("PagerNext") ?>" width="16" height="16" style="border: 0;"></a></td>	
	<?php } else { ?>
	<td><img src="phpimages/nextdisab.gif" alt="<?php echo $Language->Phrase("PagerNext") ?>" width="16" height="16" style="border: 0;"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($tbl_program_view->Pager->LastButton->Enabled) { ?>
	<td><a href="<?php echo $tbl_program_view->PageUrl() ?>start=<?php echo $tbl_program_view->Pager->LastButton->Start ?>"><img src="phpimages/last.gif" alt="<?php echo $Language->Phrase("PagerLast") ?>" width="16" height="16" style="border: 0;"></a></td>	
	<?php } else { ?>
	<td><img src="phpimages/lastdisab.gif" alt="<?php echo $Language->Phrase("PagerLast") ?>" width="16" height="16" style="border: 0;"></td>
	<?php } ?>
	<td><span class="phpmaker">&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $tbl_program_view->Pager->PageCount ?></span></td>
	</tr></tbody></table>
<?php } else { ?>
	<?php if ($tbl_program_view->SearchWhere == "0=101") { ?>
...
<br>
<?php } ?>
<form name="ftbl_programview" id="ftbl_programview" class="ewForm" action="" method="post">
<input type="hidden" name="t" value="tbl_program">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_tbl_programview" class="ewTable">
...
<?php echo $tbl_program->program_id->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
...
<?php echo $tbl_program->program_name->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
...
<?php echo $tbl_program->program_credits->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
...
<?php echo $tbl_program->program_year->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
...
<?php echo $tbl_program->program_active->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
...
<?php echo $tbl_program->program_code->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
</table>
...
<br>
<script type="text/javascript">
ftbl_programview.Init();
</script>
<?php
$tbl_program_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($tbl_program->Export == "") { ?>
...
<?php
$tbl_program_view->Page_Terminate();
?>
