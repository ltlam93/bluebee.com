...
$tbl_faculty_add = NULL; // Initialize page object first

class ctbl_faculty_add extends ctbl_faculty {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["faculty_id"] != "") {
				...
				$this->setKey("faculty_id", $this->faculty_id->CurrentValue); // Set up key
			} else {
				$this->setKey("faculty_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "tbl_facultyview.php")
	...
	function LoadDefaultValues() {
		$this->faculty_university->CurrentValue = NULL;
		$this->faculty_university->OldValue = $this->faculty_university->CurrentValue;
		$this->faculty_name->CurrentValue = NULL;
		$this->faculty_name->OldValue = $this->faculty_name->CurrentValue;
		$this->faculty_code->CurrentValue = NULL;
		$this->faculty_code->OldValue = $this->faculty_code->CurrentValue;
		$this->faculty_active->CurrentValue = NULL;
		$this->faculty_active->OldValue = $this->faculty_active->CurrentValue;
		$this->faculty_research->CurrentValue = NULL;
		$this->faculty_research->OldValue = $this->faculty_research->CurrentValue;
		$this->faculty_lab->CurrentValue = NULL;
		$this->faculty_lab->OldValue = $this->faculty_lab->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// faculty_university
		$this->faculty_university->SetDbValueDef($rsnew, $this->faculty_university->CurrentValue, NULL, FALSE);

		// faculty_name
		$this->faculty_name->SetDbValueDef($rsnew, $this->faculty_name->CurrentValue, NULL, FALSE);

		// faculty_code
		$this->faculty_code->SetDbValueDef($rsnew, $this->faculty_code->CurrentValue, NULL, FALSE);

		// faculty_active
		$this->faculty_active->SetDbValueDef($rsnew, $this->faculty_active->CurrentValue, NULL, FALSE);

		// faculty_research
		$this->faculty_research->SetDbValueDef($rsnew, $this->faculty_research->CurrentValue, NULL, FALSE);

		// faculty_lab
		$this->faculty_lab->SetDbValueDef($rsnew, $this->faculty_lab->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$this->faculty_id->setDbValue($conn->Insert_ID());
			$rsnew['faculty_id'] = $this->faculty_id->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($tbl_faculty_add)) $tbl_faculty_add = new ctbl_faculty_add();
...
// Page object
var tbl_faculty_add = new ew_Page("tbl_faculty_add");
tbl_faculty_add.PageID = "add"; // Page ID
var EW_PAGE_ID = tbl_faculty_add.PageID; // For backward compatibility

// Form object
var ftbl_facultyadd = new ew_Form("ftbl_facultyadd");

// Validate form
ftbl_facultyadd.Validate = function(fobj) {
...
ftbl_facultyadd.Form_CustomValidate =  
...
ftbl_facultyadd.ValidateRequired = true;
<?php } else { ?>
ftbl_facultyadd.ValidateRequired = false; 
<?php } ?>
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $tbl_faculty->TableCaption() ?></span></p>
...
?>
<form name="ftbl_facultyadd" id="ftbl_facultyadd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="tbl_faculty">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_tbl_facultyadd" class="ewTable">
...
<script type="text/javascript">
ew_CreateEditor("ftbl_facultyadd", "x_faculty_research", 35, 4, <?php echo ($tbl_faculty->faculty_research->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
...
<script type="text/javascript">
ew_CreateEditor("ftbl_facultyadd", "x_faculty_lab", 35, 4, <?php echo ($tbl_faculty->faculty_lab->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
