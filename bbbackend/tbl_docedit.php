...
$tbl_doc_edit = NULL; // Initialize page object first

class ctbl_doc_edit extends ctbl_doc {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->doc_id->CurrentValue == "")
			$this->Page_Terminate("tbl_doclist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// doc_name
			$this->doc_name->SetDbValueDef($rsnew, $this->doc_name->CurrentValue, NULL, $this->doc_name->ReadOnly);

			// doc_scribd_id
			$this->doc_scribd_id->SetDbValueDef($rsnew, $this->doc_scribd_id->CurrentValue, NULL, $this->doc_scribd_id->ReadOnly);

			// doc_description
			$this->doc_description->SetDbValueDef($rsnew, $this->doc_description->CurrentValue, NULL, $this->doc_description->ReadOnly);

			// doc_title
			$this->doc_title->SetDbValueDef($rsnew, $this->doc_title->CurrentValue, NULL, $this->doc_title->ReadOnly);

			// doc_status
			$this->doc_status->SetDbValueDef($rsnew, $this->doc_status->CurrentValue, NULL, $this->doc_status->ReadOnly);

			// doc_author
			$this->doc_author->SetDbValueDef($rsnew, $this->doc_author->CurrentValue, NULL, $this->doc_author->ReadOnly);

			// doc_type
			$this->doc_type->SetDbValueDef($rsnew, $this->doc_type->CurrentValue, NULL, $this->doc_type->ReadOnly);

			// doc_path
			$this->doc_path->SetDbValueDef($rsnew, $this->doc_path->CurrentValue, NULL, $this->doc_path->ReadOnly);
			...
			// doc_author_name
			$this->doc_author_name->SetDbValueDef($rsnew, $this->doc_author_name->CurrentValue, NULL, $this->doc_author_name->ReadOnly);

			// doc_publisher
			$this->doc_publisher->SetDbValueDef($rsnew, $this->doc_publisher->CurrentValue, NULL, $this->doc_publisher->ReadOnly);

			// subject_general_faculty_id
			$this->subject_general_faculty_id->SetDbValueDef($rsnew, $this->subject_general_faculty_id->CurrentValue, 0, $this->subject_general_faculty_id->ReadOnly);
...
// Create page object
if (!isset($tbl_doc_edit)) $tbl_doc_edit = new ctbl_doc_edit();

// Page init
$tbl_doc_edit->Page_Init();

// Page main
$tbl_doc_edit->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var tbl_doc_edit = new ew_Page("tbl_doc_edit");
tbl_doc_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = tbl_doc_edit.PageID; // For backward compatibility

// Form object
var ftbl_docedit = new ew_Form("ftbl_docedit");

// Validate form
ftbl_docedit.Validate = function(fobj) {
...
ftbl_docedit.Form_CustomValidate =  
...
ftbl_docedit.ValidateRequired = true;
<?php } else { ?>
ftbl_docedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftbl_docedit.Lists["x_subject_dept"] = {"LinkField":"x_dept_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_dept_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
ftbl_docedit.Lists["x_subject_type"] = {"LinkField":"x_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_subject_type_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
ftbl_docedit.Lists["x_subject_faculty"] = {"LinkField":"x_faculty_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_faculty_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
ftbl_docedit.Lists["x_subject_general_faculty_id"] = {"LinkField":"x_faculty_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_faculty_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $tbl_doc->TableCaption() ?></span></p>
<p class="phpmaker"><a href="<?php echo $tbl_doc->getReturnUrl() ?>" id="a_GoBack" class="ewLink"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $tbl_doc_edit->ShowPageHeader(); ?>
<?php
$tbl_doc_edit->ShowMessage();
?>
<form name="ftbl_docedit" id="ftbl_docedit" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="tbl_doc">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_tbl_docedit" class="ewTable">
...
<?php echo $tbl_doc->doc_id->EditValue ?></span>
<input type="hidden" name="x_doc_id" id="x_doc_id" value="<?php echo ew_HtmlEncode($tbl_doc->doc_id->CurrentValue) ?>">
</span><?php echo $tbl_doc->doc_id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
ftbl_docedit.Lists["x_subject_dept"].Options = <?php echo (is_array($tbl_doc->subject_dept->EditValue)) ? ew_ArrayToJson($tbl_doc->subject_dept->EditValue, 1) : "[]" ?>;
</script>
...
<script type="text/javascript">
ftbl_docedit.Lists["x_subject_type"].Options = <?php echo (is_array($tbl_doc->subject_type->EditValue)) ? ew_ArrayToJson($tbl_doc->subject_type->EditValue, 1) : "[]" ?>;
</script>
...
<script type="text/javascript">
ftbl_docedit.Lists["x_subject_faculty"].Options = <?php echo (is_array($tbl_doc->subject_faculty->EditValue)) ? ew_ArrayToJson($tbl_doc->subject_faculty->EditValue, 1) : "[]" ?>;
</script>
...
<script type="text/javascript">
ftbl_docedit.Lists["x_subject_general_faculty_id"].Options = <?php echo (is_array($tbl_doc->subject_general_faculty_id->EditValue)) ? ew_ArrayToJson($tbl_doc->subject_general_faculty_id->EditValue, 1) : "[]" ?>;
</script>
